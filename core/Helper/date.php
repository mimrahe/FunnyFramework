...
namespace Core\Helper
{
	class Date
	{
		...
		public static function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$is_dst=-1){
		 $h=self::tr_num($h); $m=self::tr_num($m); $s=self::tr_num($s); $jm=self::tr_num($jm); $jd=self::tr_num($jd); $jy=self::tr_num($jy);
		...
		public static function jcheckdate($jm,$jd,$jy){
		 $jm=self::tr_num($jm); $jd=self::tr_num($jd); $jy=self::tr_num($jy);
		...
		public static function gregorian_to_jalali($gy,$gm,$gd,$mod=''){
			$gy=self::tr_num($gy); $gm=self::tr_num($gm); $gd=self::tr_num($gd);/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
		...
		public static function jalali_to_gregorian($jy,$jm,$jd,$mod=''){
			$jy=self::tr_num($jy); $jm=self::tr_num($jm); $jd=self::tr_num($jd);/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
		 $gy=($jy<=979)?621:1600;
		 $jy-=($jy<=979)?0:979;
